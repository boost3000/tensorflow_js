...
(async () => {
    try {
        ...
        fs.writeFileSync(`./datasets/ratings_texts.json`, JSON.stringify(all_ratings, null, 2));
    } catch (e) {
        console.error(e);
    } finally {
        process.exit();
    ...
    try {
        ...
        if (sentiment === 'good') {
            ...
            while(ratings.length <= 500) {
                await wait(second * 5);
                const acc_ratings = await page.$$eval(' main > div section  article > div > section > div > p', els => els.map(el => el['textContent']));
                for (const rating of acc_ratings) {
                    if (rating.startsWith('Datum der Erfahrung')) {
                        continue;
                    }
